...
// Player-value labels belong in this source file, not only in generated SaveTool.html.
const AG_PLAYER_VALUES_KEYS = [
  ["ag_player_values_0", "货币", ""],
  ["ag_player_values_1", "单位上限", ""],
  ["ag_player_values_2", "武器上限", ""],
  ["ag_player_values_3", "玩家等级", ""],
  ["ag_player_values_4", "玩家经验值", ""],
  ["ag_player_values_5", "玩家升级所需的经验值", ""],
  ["ag_player_values_6", "磁盘", ""],
  ["ag_player_values_7", "兵种升级经验", ""],
  ["ag_player_values_8", "武器升级经验", ""],
  ["ag_player_values_9", "", ""],
  ["ag_player_values_10", "", ""],
  ["ag_player_values_11", "", ""],
  ["ag_player_values_12", "特殊钥匙卡", ""]
];

const AG_PLAYER_VALUE_LABELS = new Map(
  AG_PLAYER_VALUES_KEYS
    .map(([key, label]) => {
      const m = /^ag_player_values_(\d+)$/.exec(key);
      if (!m || !label) return null;
      return [Number(m[1]), label];
    })
    .filter((x) => x)
);
...
function renderPlayerValuesForm() {
  ...
  form.innerHTML = arr.map((v, i) =>
    `<label class="field"><span>${esc(labelFor(i))}</span>` +
